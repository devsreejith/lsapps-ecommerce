...
import { IoShirtOutline, IoGameControllerOutline, IoPencilOutline, IoGiftOutline } from "react-icons/io5";
import { LinkProps } from "next/link";
import { useTranslation } from "next-i18next";
import cn from "classnames";

const iconMap: Record<string, React.ReactNode> = {
  uniform: <IoShirtOutline />,
  toys: <IoGameControllerOutline />,
  stationary: <IoPencilOutline />,
  souvenir: <IoGiftOutline />,
};
...
const IconCard: React.FC<Props> = ({
  ...
  const { name, icon, tags, productCount, slug } = item ?? {};
  ...
  return (
    ...
    >
      ...
      >
        {iconMap[slug] ? (
          <div className={cn("mb-0 flex justify-center items-center text-gray-700", {
            "mx-auto mb-4 sm:mb-6 w-2/4 sm:w-2/3 md:w-8/12 3xl:w-full text-5xl sm:text-6xl md:text-7xl":
              variant === "default",
            "mb-4 sm:mb-6 w-2/4 text-4xl": variant === "modern",
            "transform scale-[0.6] lg:scale-75 2xl:scale-85 3xl:scale-90 text-8xl":
              variant === "circle",
            "mx-auto w-[40%] lg:w-2/4 text-6xl": variant === "list",
          })}>
            {iconMap[slug]}
          </div>
        ) : (
